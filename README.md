...
`src/styles/tokens.css` owns shared fonts, text sizes, colors, and button values. Appearance follows the system color scheme by default; the footer toggle can lock light or dark. `html[data-theme]` and `light-dark()` switch the same tokens between a cream palette and the dark theme. `Layout` accepts `neutral` for residency, residents, and contact; the `html.neutral-page` overrides define those pages in both themes. Change these values here when updating several pages together.
